...
def chain_map (val, prc)
  prc.each { |ele| val = ele.call(val) }
  val
end
...
def proc_suffix (sentence, procs)
  words = sentence.split(" ")
  words.each_with_index do |word, i|
    procs.each do |prc, suffix|
      if prc.call(word)
        words[i] += suffix
      end
    end
...
def proctition_platinum (arr, *procs)
  hash = {}
  deleted = Hash.new(false)

  procs.each_with_index do |prc, i|
    val = []
    arr.each_with_index do |ele, i|
      if prc.call(ele) && !deleted[ele]
        deleted[ele] = true
        val << ele
      end
    end
