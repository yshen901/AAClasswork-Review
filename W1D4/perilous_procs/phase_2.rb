def xnor_select (arr, prc_1, prc_2)
  temp = []
  arr.each { |ele| temp << ele if prc_1.call(ele) == prc_2.call(ele) }
  temp
end
...
def filter_out! (arr, &prc)
  arr.each_with_index { |ele, i| arr.slice!(i) if prc.call(ele) }
end
# arr_1 = [10, 6, 3, 2, 5 ]
...
def multi_map (arr, n=1, &prc)
  arr.each_with_index { |ele, i| n.times { arr[i] = prc.call(arr[i]) } }
end
...
def proctition (arr, &prc)
  ...
  arr.each { |ele| prc.call(ele) ? truthy << ele : falsey << ele }
  truthy + falsey
end
