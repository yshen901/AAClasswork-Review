...
def cl_to_lr_by_name
  ...
  execute(<<-SQL)
    SELECT
      a.company, a.num, stops_a.name, stops_b.name
    FROM
      routes a
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
...
def haymarket_and_leith
  ...
  execute(<<-SQL)
    ...
    JOIN
      stops stops_b ON b.stop_id = stops_b.id
    WHERE
      (stops_a.name = 'Haymarket' AND stops_b.name = 'Leith')
    GROUP BY 
      a.company, a.num
  SQL
end
...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    JOIN stops stops_a ON a.stop_id = stops_a.id
    JOIN stops stops_b ON b.stop_id = stops_b.id
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT stops_b.name, a.company, a.num
    FROM routes a
    JOIN routes b ON (a.company = b.company AND a.num = b.num)
    JOIN stops stops_a ON a.stop_id = stops_a.id
    JOIN stops stops_b ON b.stop_id = stops_b.id
